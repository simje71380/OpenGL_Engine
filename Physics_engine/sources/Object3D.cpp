...
Object3D::Object3D(unsigned int vertices_count, Vertex *vertices, unsigned int *indices, unsigned int indices_count)
	...
	//shading
	m_shader = LoadShaders("res/shaders/3DVertexShader.glsl", "res/shaders/3DFragShader.glsl");
	GLCall(glUseProgram(m_shader)); //set drawing with this shader (vertex + frag)
